...
medias_api = Blueprint("medias_api", __name__, url_prefix='/project1/api/medias/')

@medias_api.route('/',methods=['GET','POST','PUT','DELETE'])
def media_contro(page=None, per_page=None):
    if request.method == 'GET':
        t_id = request.args.get('t_id')
        media = Medias.query.filter_by(t_id=t_id)
        if media:
            data = []
            for m in media:
        ...
        if t_id:
            ...
            if theme:
                theme_path = os.getcwd() + theme.t_url
                if m_type == '0':
                    if mainpic:
                        ...
                        try:
                            ...
                            media.m_url = file_path.split("clint_dev")[1]
                        ...
                        return jsonify({'msg': 'not file'}), 406
                if m_type == '1':
                    if video:
            ...
            return jsonify({'msg': 't_id not found'}), 400
